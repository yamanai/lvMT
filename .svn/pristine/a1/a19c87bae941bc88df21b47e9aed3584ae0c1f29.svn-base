...
import * as mutationNames from '../../vuex/models/flightorder/MutationName.js'
	...
	export default{
		...
		mounted(){
            ...
            let url = 'http://local.happyeasygo.com:8081/indi_demo/order/getFlights.do?token=' + orderToken;
            let dNumber = this.getFlightNumber('departFlight');
            let rNumber = [];
            if(!orderToken){
                this.isShow = !this.isShow;
                Toast({
                    message:'身份信息已过期，即将返回首页',
                    duration: 2000
                })
                setTimeout(function(){
...
//            let jf =  new Promise((resolve,reject) => {
...
//                }).catch(err => reject(err))
//            })
//            jf.then(value => console.log(value))
