...
export class FlightOrder extends SerializableObject{
    constructor(departAirport, destinationAirport, departDate, adults = [], children = [], infants = [], cabinClass, contactInfo,baggageinfo=[],serviceFee,orderNumber,token) {
        super()
    ...
    checkPrice(vueComp) {
        ...
        return new Promise((resolve, reject) => {
            vueComp.$http.post(DomainManager.getPriceCheckUrlString(), json).then(req => {
                req.json().then(json => {
